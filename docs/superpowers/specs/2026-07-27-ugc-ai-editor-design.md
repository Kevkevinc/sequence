...
- **Reference-video style transfer (candidate Stage 2 feature, proposed 2026-07-27):** let a creator upload a well-performing TikTok Shop UGC video they like, have the app analyze its editing pattern (pacing, cut structure, hook style/placement, overlay timing), and apply that same pattern to the creator's own raw clips instead of only drawing from the built-in hook-archetype library. The user has sample high-performing videos available to provide for this. Needs its own brainstorming pass when Stage 2 (AI editing pipeline) design starts — worth scoping how "analyze a reference video" actually works (vision model shot-segmentation of the reference, versus a simpler heuristic like cut-timing/hook-position extraction) and how it composes with the existing director-step architecture.
